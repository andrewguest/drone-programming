...
async def precheck_battery(drone: System):
    ...
    console.print(f"[{blue}]Battery voltage:[/{blue}] [{white}]{round(battery_info.voltage_v, 2)}v[/{white}]")
    console.print(f"[{blue}]Battery percentage:[/{blue}] [{white}]{battery_info.remaining_percent} %[/{white}]")
...
async def precheck_connection(drone: System):
    ...
    if connection_state.is_connected:
        console.print(f"[{blue}]Connection state:[/{blue}] [{green}]Connected[/{green}]")
    else:
...
async def precheck_gps(drone):
    ...
    console.print(f"[{blue}]Number of GPS satellites:[/{blue}] [{white}]{gps.num_satellites}[/{white}]")
...
async def precheck_position(drone):
    ...
    console.print(f"[{blue}]Latitude:[/{blue}] [{white}]{position.latitude_deg:.4f}[/{white}]")
    console.print(f"[{blue}]Longitude:[/{blue}] [{white}]{position.longitude_deg:.4f}[/{white}]")
    console.print(
